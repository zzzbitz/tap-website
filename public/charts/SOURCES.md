...
| PrepBench cost and accuracy | Lupi Basics / F8 Plumb Scatter    | templates/basics-gallery.html | Price against satisfaction, twelve products | Exact point positions with a computed Pareto frontier, selected-point guides, numeric axes, responsive geometry, numbered model key, keyboard/touch selection and exact-value readout. Original plumb lines and barcode floor removed. |
...
model is dominated when another model costs no more and is at least as accurate,
with at least one strict improvement. The blue line joins the remaining models
in cost order; it connects observed results and does not infer intermediate
runs. Prep-Code has four frontier models; GUI Workflow has five. The faint
background wash is decorative. Only the selected model shows axis guides.
